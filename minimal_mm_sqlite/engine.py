...
class Engine:
    ...
    def build(cls) -> "Engine":
        ...
        # WS market-data signing (required even in paper mode).
        key_id = (os.getenv("PROD_KEYID") or "").strip()
        keyfile = (os.getenv("PROD_KEYFILE") or "").strip()
        if not key_id or not keyfile:
            raise RuntimeError("Missing PROD_KEYID/PROD_KEYFILE (required for WebSocket market data)")
        ...
        return cls(cfg=cfg, db=db, writer=writer, md=md, selector=selector, exec=exec_client, working={})
